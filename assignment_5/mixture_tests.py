...
class GMMTests(unittest.TestCase):
    ...
    def bonus_test(self, bonus):
        def naive_bonus(X, means):
            m, n = X.shape
            k, _ = means.shape
            dist = np.zeros((m, k))
            for _m in range(m):
                for _k in range(k):
                    for _n in range(n):
                        dist[_m, _k] += (X[_m,_n] - means[_k, _n])**2
                    dist[_m, _k] = np.sqrt(dist[_m, _k])
            return dist

        X = np.random.randn(5, 3) # np.array([[2,0],[0,1]])
        Y = np.random.randn(5, 3) # np.array([[1,0],[0,1]])

        naive_res = naive_bonus(X, Y)
        res = bonus(X, Y)
        print(naive_res)
        print(res)
        if np.all(res == naive_res):
            print("yay")
        else:
            print("Boo")
