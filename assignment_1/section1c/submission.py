...
class CustomEvalFn:
    ...
    def score(self, game, my_player=None):
        ...
        op_pos = game.get_opponent_position(my_player)
        return self.w1*len(my_moves)-self.w2*len(op_moves) * (abs(op_pos[0]-3)+abs(op_pos[1]-3)-abs(my_pos[0]-3)-abs(my_pos[1]-3)/5)
        # TODO: finish this function!
        # raise NotImplementedError
...
class CustomPlayer:
    ...
    def __init__(self, search_depth=6, eval_fn=CustomEvalFn(), w1 = 1, w2 = 1):
...
#         best_move, utility = alphabeta(self, game, time_left, depth=1)
#         if time_left() < 2:
#             return best_move
# #         pdb.set_trace()
#         best_move, utility = alphabeta(self, game, time_left, depth=2)
#         if time_left() < 2:
#             return best_move
# #         pdb.set_trace()
        best_move, utility = alphabeta(self, game, time_left, depth=3)
        if time_left() < 2:
            return best_move
        best_move, utility = alphabeta(self, game, time_left, depth=4)
        if time_left() < 2:
            return best_move
        best_move, utility = alphabeta(self, game, time_left, depth=5)
        if time_left() < 2:
            return best_move
        best_move, utility = alphabeta(self, game, time_left, depth=6)
        if time_left() < 2:
            return best_move
        best_move, utility = alphabeta(self, game, time_left, depth=7)
        if time_left() < 2:
            return best_move
        best_move, utility = alphabeta(self, game, time_left, depth=8)
        if time_left() < 2:
            return best_move
        best_move, utility = alphabeta(self, game, time_left, depth=9)
        if time_left() < 2:
            return best_move
...
#             # print(best_move, game.move_count, depth)
#             if time_left() < 5:
#                 return best_move
...
def max_value(player, game, time_left, max_depth, curr_depth, alpha, beta, my_turn):
    if curr_depth == max_depth:
        ...
        v = float("-inf")
        key = str(game.__board_state__)+str(game.get_active_player())+str(game.SWAP_FLAG)
        if key not in player.move_cache:
            moves = game.get_player_moves(game.get_active_player())
            move_list = np.zeros((len(moves), 4), dtype=np.int32)
            move_list[:,0] = [-50]*len(moves)
            move_list[:, 1:4] = moves
            player.move_cache[key] = move_list
        else:
            player.move_cache[key]= player.move_cache[key][(-1*player.move_cache[key][:,0]).argsort()]

        # legal_moves = game.get_player_moves(game.get_active_player())
        for idx, move_pair in enumerate(player.move_cache[key]):
            _, r, c, sw = move_pair
            move = (r, c, bool(sw))

            new_board, is_over, is_winner = game.forecast_move(move)
            if is_over:
                player.move_cache[key][idx, 0] = 50
                return move, +50

            if time_left() < 50:
                val = player.eval_fn.score(new_board, new_board.get_active_player())
                player.move_cache[key][idx, 0] = val
                return move, val
...
def min_value(player, game, time_left, max_depth, curr_depth, alpha, beta, my_turn):
    if curr_depth == max_depth:
        ...
        if key not in player.move_cache:
            ...
            player.move_cache[key]= player.move_cache[key][player.move_cache[key][:,0].argsort()]
        ...
        for idx, move_pair in enumerate(player.move_cache[key]):
            ...
            if time_left() < 50:
                ...
                val = -1*player.eval_fn.score(new_board, new_board.get_active_player())
                player.move_cache[key][idx, 0] = val
                return move, val
