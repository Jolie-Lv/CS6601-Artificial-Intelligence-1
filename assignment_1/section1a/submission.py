...
def dfs(player, game, time_left, max_depth, depth, my_turn):
    # Init
    best_move, best_val = None, -float("inf")

    # what moves are available to active player
    moves = game.get_player_moves(game.get_active_player())

    ## if no moves left for active player
    # if len(moves) == 0:
    #     # Score is -infinity because my player lost
    #     best_move, best_val = (-1,-1,False), -float("inf")

    # if max_depth is reached,
    if depth == max_depth:
        best_val = player.eval_fn.score(game, game.get_active_player())
        if not my_turn: best_val = best_val * -1

    # if max depth is not reached
    else:
        if my_turn: best_val = -float("inf")
        else: best_val = float("inf")
        for move in moves:
            new_board, is_over, is_winner = game.forecast_move(move)
            if is_over or time_left() < 100:
                val = player.eval_fn.score(new_board, new_board.get_active_player())
                if not my_turn: val = val * -1
            else:
                _, val = dfs(player, new_board,time_left, max_depth, depth+1, (not my_turn))
...
def minimax(player, game, time_left, depth, my_turn=True):
    ...
    # TODO: finish this function!
    # raise NotImplementedError
    best_move, best_val = dfs(player, game, time_left, depth, 0, my_turn=my_turn)
    return best_move, best_val
