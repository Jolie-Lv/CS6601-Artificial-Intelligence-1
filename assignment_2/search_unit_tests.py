...
class SearchUnitTests(unittest.TestCase):
    ...
    def run_romania_tri(self, method, **kwargs):
        ...
        for goals in triplets:
            ...
            s1len, mp1 = self.reference_path(self.romania, goals[0], goals[1])
            s2len, mp2 = self.reference_path(self.romania, goals[2], goals[1])
            s3len, mp3 = self.reference_path(self.romania, goals[0], goals[2])
            min_len = min(s1len + s2len, s1len + s3len, s3len + s2len)

            if path_len != min_len:
                print("My Path: ", path, path_len)
                print("Reference: ", mp1, mp2, mp3, min_len)
                print (goals)
    ...
    def test_tri_a_star_romania(self):
        """Test Tri-UC search with Romania data."""
